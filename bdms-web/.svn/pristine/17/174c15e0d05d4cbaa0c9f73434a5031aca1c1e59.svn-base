						...
						$.getJSON("${APP_PATH}dams/metro/in/stationsbyline/"+lineIdParam, { "resultType": "json" }, function(data, textStatus)
								{
								   $("#selectmenu2").empty();//先清空tbody  
									...
									$("#selectmenu2").selectmenu("refresh");
									$("#lineStatInfo").html("<span style='color:"+lineNoCo[parseInt(lineIdParam)]+"'>"+lineIdParam+"</span><span "+style+"> 号线 </span><span style='color:"+lineNoCo[parseInt(lineIdParam)]+"'>"+sName+"</span><span "+style+"> 出站人数</span>");
								});
					}
			...
			$("#selectmenu").on("selectmenuchange",function( event, ui){
				...
				if(linenum!="-"){
					...
					if(linenum == "00"){
						...
						$("#lineStatInfo").html("<span "+style+">换乘线 出站人数  </span>");
						...
						$("#lineStatInfo").html("<span style='height:50px;color;"+lineNoCo[parseInt(str)]+"'>"+str+"</span><span "+style+"> 号线 出站人数 </span>");
						$('#lineStatInfo').css("display","none");
						$('#lineStatInfo').fadeIn(200);
					...
					//clearallinfo();
					//changeTitleBg("top");
					$("#lineStatInfo").html("<span "+style+">全市 出站人数</span>");
					$('#lineStatInfo').css("display","none");
					$('#lineStatInfo').fadeIn(200);
				...
				$.getJSON("${APP_PATH}dams/metro/in/station/"+stat, { "resultType": "json" }, function(data, textStatus)
					{
						if(stat!="-"){
							if(linenum!="00"){
								$("#lineStatInfo").html("<span style='color:"+lineNoCo[parseInt(str)]+"'>"+str+"</span><span "+style+"> 号线 </span><span style='color:"+lineNoCo[parseInt(str)]+"'>"+data.name+"</span><span "+style+"> 出站人数</span>");
							}else{
								$("#lineStatInfo").html("</span><span "+style+">换乘线  </span><span style='color:"+lineNoCo[parseInt(str)]+"'>"+data.name+"</span><span "+style+"> 出站人数</span>");
							}
							//changeStationLogo(data);
							changeStationImgData(stat);
						}else{
							if(linenum!="00"){
								$("#lineStatInfo").html("<span style='color:"+lineNoCo[parseInt(str)]+"'>"+str+"</span><span "+style+"> 号线 </span><span "+style+"> 出站人数</span>");
							}else{
								$("#lineStatInfo").html("</span><span "+style+">换乘线  </span><span "+style+"> 出站人数</span>");
							}
							changeStationImgData(linenum+"00");
						}
					});
...
function refreshPNums(num1,num2,num3,num4,num12,num22,num32,num42,pSize){
 ...
 function refreshPNum(id,num,numh,pSize){
	 ...
	 if(stat2!="-"){
	 	codeStr=stat2;
	 	typeStr="stationExitRT";
	 }else if(stat!="-"){
		 if(stat="00"){
			 codeStr="0098";
			 typeStr="transLineExitRT";
		 }else{
			 codeStr=stat; 
			 typeStr="lineExitRT"; 
		 }
...
<div class="ui-layout-center west-body-background">
	<div style="width:100%; height:100px;">
		<div id="lineStatInfo"><span class="font-family-color">全市 出站人数</span></div>
	</div>
	...
	<div id="right" style="float:left;width:380px;padding-top:10px">
		...
		<div style="width: 380px ;margin-top: 30px;font-family: '微软雅黑';">
			...
			<div style="width:100%;BORDER-TOP: #00686b 1px dashed; OVERFLOW: hidden; HEIGHT: 20px"></div>
				...
				<div style="width:200px; float:left;font-weight:bold;padding-left:10px;font-size:18px" class="just-font">出站人数</div>
			</div>
			...
			<!-- <div style="width: 100%; padding-top: 20px;float:left">
      		...
      		<div style="width: 100%; padding-top: 20px;float:left" >
				<div style="float:left;margin-top:1px;margin-left:10px"><img src="${APP_PATH}style/jqueryUIthems/${THEME}/images/round.png"></img></div>
				<div style="width:200px; float:left;font-weight:bold;padding-left:10px;font-size:18px" class="just-font">出站总人数</div>
			</div>
			<div class="renPlusNum" >
...
var yc = "${APP_PATH}dams/metro/out/dayexitpredictedata";
var station = "0099";
		...
		hcharts = new Highcharts.Chart({ 
         ...
         formatter: function () {  
	        ...
	        series:[
	        	...
	        	{name:"出站",data:out,color:'rgb(0,84,248)'}
	        ]
	    });
	}
