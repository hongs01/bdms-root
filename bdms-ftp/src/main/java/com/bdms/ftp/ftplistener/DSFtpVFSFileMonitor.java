 ...
 */
public class DSFtpVFSFileMonitor implements Runnable, FileMonitor
{
    private static final Logger LOG = LoggerFactory.getLogger(DSFtpVFSFileMonitor.class);
    ...
    public DSFtpVFSFileMonitor(final FileListener listener)
    {
    ...
    private static final class FileMonitorAgent
    {
        ...
        private final DSFtpVFSFileMonitor fm;
        ...
        private FileMonitorAgent(DSFtpVFSFileMonitor fm, FileObject file)
        {
            this.fm = fm;
            this.file = file;
            ...
            if (this.exists)
            {
                try
                {
                	if( "folder".equals( this.file.getType().getName() ) ){
                		this.timestamp = this.file.getContent().getLastModifiedTime();
                	}else{
                		this.fileSize = this.file.getContent().getSize();
                		this.timestamp = this.file.getContent().getLastModifiedTime();
                	}
                }
                catch (FileSystemException fse)
            ...
            try
            {
                ...
                else if (this.exists && this.file.exists())
                {
                	if( "folder".equals( this.file.getType().getName() ) ){
	                    ...
	                    // Check the timestamp to see if it has been modified
	                    if ( (this.fileSize != this.file.getContent().getSize()) || (this.timestamp != this.file.getContent().getLastModifiedTime() ))
	                    {
	                        this.fileSize = this.file.getContent().getSize();
	                        this.timestamp = this.file.getContent().getLastModifiedTime();
	                        // Fire change event
